##' \code{\link{compare_plans_dilution_2_pd_betabinom}} provides graphical displays of the probability of the detection curves for dilution schemes in the first dilution stage.
...
##' @details \code{\link{compare_plans_dilution_2_pd_betabinom}} provides graphical displays of the probability of the detection curves for dilution schemes in the first dilution stage.
...
##' alpha <- 1
##' beta <- 5
##' compare_plans_dilution_2_pd_betabinom(S, V0, V1, V2, V3, n_sim, Clim_lower, Clim_upper, alpha, beta)
##' @usage  compare_plans_dilution_2_pd_betabinom(S, V0, V1, V2, V3, n_sim, Clim_lower, Clim_upper, alpha, beta)
##' @export
compare_plans_dilution_2_pd_betabinom <- function(S, V0, V1, V2, V3, n_sim, Clim_lower, Clim_upper, alpha, beta) {
  ...
  prob_detect_dilution_2_multi_betabinom <- function(S, V0, V1, V2, V3, C_cfu, n_sim, alpha, beta) {
    pd <- matrix(NA, nrow = 1, ncol = length(C_cfu))
    for (j in 1:length(C_cfu)) {
      pd[1, j] <- prob_detect_dilution_2_betabinom(S, V0, V1, V2, V3, C_cfu[j], n_sim, alpha, beta)
    }
  ...
  for (j in 1:length(S)) {
    pd[, j] <- prob_detect_dilution_2_multi_betabinom(S[j], V0, V1, V2, V3, C_cfu, n_sim, alpha, beta)
  }
