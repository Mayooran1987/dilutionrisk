...
#'
#' @export
pd_curves_heterogeneous <- function(mu_low, mu_high, sd, a, b, f, u, USL,
  ...
  # Calculate probability of detection for each mu and scheme
  pd_matrix <- matrix(NA, nrow = length(mu_seq), ncol = length(f))
  ...
  for (i in seq_along(mu_seq)) {
    pd_matrix[i, ] <- prob_detection_heterogeneous_multiple(
      mu_seq[i], sd, a, b, f, u, USL, type, n_sim
    )
  ...
  Prob <- data.frame(mu = mu_seq, pd_matrix)
