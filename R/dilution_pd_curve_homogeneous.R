##' \code{\link{dilution_pd_curve_homogeneous}} provides the probability of detection curves when the diluted sample has homogeneous contaminants.
...
##' @details \code{\link{dilution_pd_curve_homogeneous}} provides probability of detection curves for different dilution schemes when the diluted sample has homogeneous contaminants (this section will be updated later on).
...
##' USL <- c(1000, 2000)
##' n_sim <- 10000
##' dilution_pd_curve_homogeneous(lambda_low, lambda_high, a, b, pf, USL, n_sim)
##' @usage  dilution_pd_curve_homogeneous(lambda_low, lambda_high, a, b, pf, USL, n_sim)
dilution_pd_curve_homogeneous <- function(lambda_low, lambda_high, a, b, pf, USL, n_sim){
  p_d <- NULL
  ...
  for (i in 1:length(lambda)) {
    Pd[i,1] <-  prob_detection_homogeneous(lambda[i], a, b, pf, USL[1], n_sim)
    Pd[i,2] <-  prob_detection_homogeneous(lambda[i], a, b, pf, USL[2], n_sim)
  }
