...
##' @usage  prob_detection_heterogeneous_multiple(mu, sd, a, b, f, u, USL, n_sim)
##' @export
prob_detection_heterogeneous_multiple <- function(mu, sd, a, b, f, u, USL, n_sim){
  if (length(f) != length(u)) stop("please use equal length of f and u", call. = FALSE)
  sim1 <- matrix(NA, nrow =  1, ncol = length(f))
  for (i in 1:length(f)) {
    sim1[,i] <-   prob_detection_heterogeneous(mu, sd, a, b, f[i], u[i], USL, n_sim)
  }
