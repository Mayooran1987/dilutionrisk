...
cv_homogeneous_multiple <- function(lambda, a, b, f, u, USL, n_sim) {
  ...
  results <- vapply(seq_along(f), function(i) {
    cv_homogeneous(lambda, a, b, f[i], u[i], USL, n_sim)
  }, numeric(1))
...
cv_curves_homogeneous <- function(lambda_low, lambda_high, a, b, f, u, USL, n_sim) {
  ...
  if (!is.numeric(lambda_high) || length(lambda_high) != 1 || lambda_high <= lambda_low) {
  ...
  # Calculate CV for each lambda
  cv_matrix <- matrix(NA, nrow = length(lambda_seq), ncol = length(f))

  for (i in seq_along(lambda_seq)) {
    cv_matrix[i, ] <- cv_homogeneous_multiple(
  ...
  Prob <- data.frame(lambda = lambda_seq, cv_matrix)
