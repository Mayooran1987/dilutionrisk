...
##' n <- 5
##' n_sim <- 50000
##' prob_acceptance_heterogeneous_multiple (c, mu, sd, a, b, f, u, USL, n, n_sim)
##' @usage  prob_acceptance_heterogeneous_multiple (c, mu, sd, a, b, f, u, USL, n, n_sim)
##' @export
prob_acceptance_heterogeneous_multiple <- function(c, mu, sd, a, b, f, u, USL, n, n_sim){
  sim2 <- NULL
  if (length(f) != length(u)) stop("please use equal length of f and u", call. = FALSE)
  sim1 <- matrix(NA, nrow =  1, ncol = length(f))
  for (i in 1:length(f)) {
    sim2[i] <-  prob_detection_heterogeneous_multiple(mu, sd, a, b, f[i], u[i], USL, n_sim)
