...
##' @param mu_low the lower value of the mean concentration (\eqn{\mu}) for use in the graphical display's x-axis (on the log scale).
##' @param mu_high the upper value of the mean concentration (\eqn{\mu}) for use in the graphical display's x-axis (on the log scale).
...
##' @usage  pd_curves_heterogeneous(mu_low, mu_high, sd, a, b, f, u, USL, n_sim)
##' @export
pd_curves_heterogeneous <- function(mu_low, mu_high, sd = 0.8, a, b, f, u, USL, n_sim){
  p_d <- NULL
  ...
  for (i in 1:length(mu)) {
    Pd[i,] <-  cbind(prob_detection_heterogeneous_multiple(mu[i], sd, a, b, f, u, USL, n_sim))
  }
  ...
  plot_sam <- ggplot2::ggplot(melten.Prob) + ggplot2::geom_line(ggplot2::aes(x = mu, y = p_d, group = Dilution_scheme, colour = Dilution_scheme)) +
    # ggplot2::ggtitle("OC curve based on Lognormal distribution") +
    ggplot2::theme_classic() + ggplot2::xlab(expression(" log mean concentrations  (" ~ mu*~")")) + ggplot2::ylab(expression("Probability of detection"~(P[d]))) + ggthemes::scale_colour_colorblind() +
