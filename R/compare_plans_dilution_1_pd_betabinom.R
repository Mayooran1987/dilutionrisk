...
##' @details \code{\link{compare_plans_dilution_1_pd_betabinom}} provides graphical displays of the probability of the detection curves for dilution schemes in the first dilution stage testing.
...
##' @usage  compare_plans_dilution_1_pd_betabinom(S, V0, V1, Clim_lower, Clim_upper, alpha, beta)
##' @export
compare_plans_dilution_1_pd_betabinom <- function(S, V0, V1, Clim_lower, Clim_upper, alpha, beta) {
  Sampling_scheme <- NULL # Initalizing
  p_d <- NULL
  C <- NULL
  C_cfu <- seq(Clim_lower, Clim_upper, by = 0.1)
  f_spr <- function(S) {
    sprintf("sample weight (%.0f gram)", S)
  }
  pd <- matrix(NA, nrow = length(C_cfu), ncol = length(S))
  for (j in 1:length(S)) {
    pd[, j] <- prob_detect_dilution_1_betabinom(S[j], V0, V1, C_cfu, alpha, beta)
  }
