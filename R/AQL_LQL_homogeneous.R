...
AQL_LQL_homogeneous <- function(c, lambda_low,lambda_high, a, b, f, u, USL, n,  type = "theory", alpha, beta, OC, n_sim = NA){
  ...
  Prob_df <- data.frame(lambda, round(Pa,4))

  # Prob_df <- data.frame(p, Pa)
  plot_AQL_LQL <- ggplot2::ggplot(Prob_df) + ggplot2::geom_line(ggplot2::aes(x = lambda, y = Pa)) +
    ggplot2::xlab(expression("expected microbial count  (" ~ lambda*~")")) + ggplot2::ylab(expression("Probability of acceptance"~(P[a]))) + ggplot2::theme_classic() +
    ggplot2::geom_hline(yintercept = c(Pa[which(lambda == lambda[which(Pa == (1 - alpha))])]),linetype = "dashed") +
    ggplot2::geom_vline(xintercept = c(lambda[which(Pa == 1 - alpha)]),linetype = "dashed") +
    ggplot2::geom_hline(yintercept = c(Pa[which(lambda == lambda[which(Pa == beta)])]),linetype = "dashed") +
    ggplot2::geom_vline(xintercept = c(lambda[which(Pa == beta)]),linetype = "dashed") +
    ...
    ggplot2::annotate("text", x = c(100 + lambda[which(Pa == 1 - alpha)],100 + lambda[which(Pa == beta)]),
                      y = c(0,0),
                      label = sprintf(c("\n AQL = %0.4f","\n LQL = %0.4f"), c(lambda[which(Pa == 1 - alpha)],lambda[which(Pa == beta)])), size = 3) +
    ggplot2::annotate("text", x = c(100,100),
                      y = c(Pa[which(lambda == lambda[which(Pa == 1 - alpha)])],Pa[which(lambda == lambda[which(Pa == beta)])]),
                      label = sprintf(c("\n 1-\u03B1 = %0.2f","\n \u03B2 = %0.2f"), c(Pa[which(lambda == lambda[which(Pa == 1 - alpha)])],Pa[which(lambda == lambda[which(Pa == beta)])])), size = 3)
  if ( OC == TRUE) {
