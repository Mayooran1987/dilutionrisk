...
#' @param mu_low Lower bound of mean microbial count (\eqn{\mu}) for x-axis
#'   in graphical displays (log scale).
#' @param mu_high Upper bound of mean microbial count (\eqn{\mu}) for x-axis
#'   in graphical displays (log scale).
...
#' @aliases cv_heterogeneous cv_heterogeneous_multiple cv_curves_heterogeneous
...
cv_heterogeneous <- function(mu, sd, a, b, f, u, USL, n_sim) {
  ...
  for (j in seq_len(n_sim)) {
    sim1[j, ] <- rtrunpoilog(n_rep, (mu + log(fu, exp(1))), sd, a, b) *
      (1 / (f * u))
  }
