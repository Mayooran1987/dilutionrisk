...
AQL_LQL_heterogeneous <- function(c,mu_low,mu_high,sd,a,b,f,u,USL,n,type = "theory", alpha ,beta,OC,n_sim = NA){
  ...
  plot_AQL_LQL <- ggplot2::ggplot(Prob_df) + ggplot2::geom_line(ggplot2::aes(x = mu, y = Pa)) +
    ggplot2::xlab(expression("expected microbial count  (" ~ mu*~")")) + ggplot2::ylab(expression("Probability of acceptance"~(P[a]))) +
    ggplot2::theme_classic() +
    ggplot2::geom_hline(yintercept = c(Pa[which(mu == mu[which(Pa == 1 - alpha)])]),linetype = "dashed") +
    ggplot2::geom_vline(xintercept = c(mu[which(Pa == 1 - alpha)]),linetype = "dashed") +
    ggplot2::geom_hline(yintercept = c(Pa[which(mu == mu[which(Pa == beta)])]),linetype = "dashed") +
    ggplot2::geom_vline(xintercept = c(mu[which(Pa == beta)]),linetype = "dashed") +
    ...
    ggplot2::annotate("text", x = c(mu[which(Pa == 1 - alpha)],mu[which(Pa == beta)]),
                      y = c(0,0),
                      label = sprintf(c("\n AQL = %0.4f","\n LQL = %0.4f"), c(mu[which(Pa == 1 - alpha)],mu[which(Pa == beta)])), size = 3) +
    ggplot2::annotate("text", x = c(mu_low+0.5,mu_low+0.5),
                      y = c(Pa[which(mu == mu[which(Pa == 1 - alpha)])],Pa[which(mu == mu[which(Pa == beta)])]),
                      label = sprintf(c("\n 1-\u03B1 = %0.2f","\n \u03B2 = %0.2f"), c(Pa[which(mu == mu[which(Pa == 1 - alpha)])],Pa[which(mu == mu[which(Pa == beta)])])), size = 4)
  if ( OC == TRUE) {
