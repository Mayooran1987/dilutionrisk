...
#' function of the true microbial concentration. They are useful for comparing
...
OC_curves_homogeneous <- function(c, lambda_low, lambda_high, a, b, f, u,
  ...
  # Calculate probability of acceptance for each lambda and scheme
  pa_matrix <- matrix(NA, nrow = length(lambda_seq), ncol = length(f))
  ...
  for (i in seq_along(lambda_seq)) {
    pa_matrix[i, ] <- prob_acceptance_homogeneous_multiple(
      c, lambda_seq[i], a, b, f, u, USL, n, type, n_sim
    )
  ...
  Prob <- data.frame(lambda = lambda_seq, pa_matrix)
  ...
  plot_sam <- ggplot2::ggplot(melten.Prob) +
    ...
    ggplot2::annotate("text", x = USL, y = 0.05,
                      label = sprintf("USL = %0.0f", USL),
                      size = 3.5, hjust = -0.1) +
    ggplot2::theme(
